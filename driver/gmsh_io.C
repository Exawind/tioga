    ...
    int** ndc8)
{
    ...
    gmsh_size_read(
        gmsh_filename, node_num, dim, element_num, elem_counts.data());
    ...
    for (int i = 0; i < node_num; i++)
        bcall[i] =
            (node_x[3 * i + _Y] >= outerboxTop - eps); // top overset face
    for (int i = 0; i < node_num; i++)
        bcall[i] +=
            (node_x[3 * i + _X] <= -outerboxh + eps); // xlo overset face
    for (int i = 0; i < node_num; i++)
        bcall[i] += (node_x[3 * i + _X] >= outerboxh - eps); // xhi overset face
    for (int i = 0; i < node_num; i++)
        bcall[i] +=
            (node_x[3 * i + _Z] <= -outerboxh + eps); // zlo overset face
    for (int i = 0; i < node_num; i++)
        bcall[i] += (node_x[3 * i + _Z] >= outerboxh - eps); // zhi overset face
    ...
    for (;;) {
        ...
        if (level == 0) {
            ...
            if (s_begin(text, "$EndElements")) {
                ...
                int em_id = s_to_i4(text, length, ierror);
                text.erase(0, length); // element id
                elem_type = s_to_i4(text, length, ierror);
                text.erase(0, length); // element type
                s_to_i4(text, length, ierror);
                text.erase(0, length); // number of tags (2)
                s_to_i4(text, length, ierror);
                text.erase(0, length); // phys tag
                s_to_i4(text, length, ierror);
                text.erase(0, length); // entity tag

                int element_count = element_counts[getElementType(elem_type)];
                int num_nodes = getElementNumNodes(elem_type);

                // printf("Filling Elem %d of type %d: count %d, num_nodes
                // %d\n",em_id,elem_type,element_count,num_nodes);
                int* const elem_nodes =
                    &(element_nodes[getElementType(elem_type)]
                                   [num_nodes * element_count]);

                // read element nodes
                for (i = 0; i < num_nodes; i++) {
                    k = s_to_i4(text, length, ierror);
                    text.erase(0, length);
                    // element_nodes[getElementType(elem_type)][element_order*element_count+i]
                    // = k;
                    elem_nodes[i] = k;
                }
    ...
    for (;;) {
        ...
        if (level == 0) {
            ...
            if (s_begin(text, "$EndMeshFormat")) {
                ...
                if ((int)10 * fmt_a >= 30) {
                    cout << "ERROR - GMSH FORMAT 2.X ONLY: read format="
                         << fmt_a << endl;
                    exit(1);
                }
            }
