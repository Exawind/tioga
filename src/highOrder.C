...
void MeshBlock::clearOrphans(HOLEMAP* holemap, int nmesh, const int* itmp)
{
    ...
    if (ihigh != 0) {
        ...
        for (i = 0; i < nreceptorCells; i++) {
            ...
            if (reject == 1) {
                iblank_cell[ctag[i] - 1] =
                    0; // changed to hole if inside hole map
            } else if (reject == 2) {
                iblank_cell[ctag[i] - 1] =
                    1; // changed to field if not inside hole map
            }
        }
    } else {
        m = 0;
...
void MeshBlock::getExtraQueryPoints(
...
void MeshBlock::getInterpolatedSolutionAtPoints(
    int* nints,
    int* nreals,
    int** intData,
    double** realData,
...
void MeshBlock::updatePointData(
    double* q, double* qtmp, int nvar, int interptype)
{
    int i, j, k, n, m;
    double* qout;
    int index_out;
    ...
    if (ihigh != 0) {
        ...
        for (i = 0; i < nreceptorCells; i++) {
            if (iblank_cell[ctag[i] - 1] == -1) {
                convert_to_modal(
                    &(ctag[i]), &(pointsPerCell[i]), &(qtmp[m]), &npts,
                    &index_out, qout);
                index_out -= BASE;
                k = 0;
                for (j = 0; j < npts; j++) {
