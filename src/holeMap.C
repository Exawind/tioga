...
void tioga::getAdaptiveHoleMap(void){
  ...
  ADAPTIVE_HOLEMAP_OCTANT AHMOLocal;
  ...
  // determine hole for this body: global reduce wall flag first
  MPI_Allreduce(MPI_IN_PLACE,&existWallFlag,1,MPI_INT,MPI_MAX,mb->blockcomm);
  AHMOLocal.existWall = existWallFlag;
  ...
  /* ======================================================== */
  /* Step 3: flood fill adaptive hole map for this mesh-block */
  ...
  /* ================================================== */
  /* Step 4: build hole map and inform all complement   */
  /*         ranks of each mesh-block adaptive hole map */
  /* ================================================== */
  if(adaptiveHoleMap){
    for(i=0;i<nmesh;i++){
      if(adaptiveHoleMap[i].existWall){
        for(j=0;j<adaptiveHoleMap[i].meta.nlevel;j++){
          free(adaptiveHoleMap[i].levels[j].octants);
        }
      }
    }
  ...
  for(i=0;i<maxtag;i++){
    adaptiveHoleMap[i].meta.nlevel = 0;
    adaptiveHoleMap[i].meta.elem_count = 0;
    adaptiveHoleMap[i].meta.leaf_count = 0;
  }
  ...
  existHole.resize(maxtag);

  // zero array
  for(i=0;i<maxtag;i++) existHole[i] = 0;
  existHole[meshtag-BASE] = (uint8_t) existWallFlag;
  MPI_Allreduce(MPI_IN_PLACE,existHole.data(),maxtag,MPI_UINT8_T,MPI_MAX,scomm);

  // set wall flags for all hole maps
  for(i=0;i<maxtag;i++) adaptiveHoleMap[i].existWall = existHole[i];
  ...
  /* ------------------------------------------------------ */
  /* communicate adaptive hole map info to complement ranks */
  /* ------------------------------------------------------ */
  for(i=0;i<maxtag;i++){
    ...
    meta.elem_count = 0;
    if(AHME.existWall){
      meshblockCompInfo &MBC = meshblockComp[i];

      // complement + master ranks only involved
      if((i!=meshtag-BASE) || (i==meshtag-BASE && mb->meshMaster)){

        // 1. inform of meta data for map: packed into contiguous buffer
        MPI_Ibcast(&AHME.meta,
                   sizeof(ahm_meta_t),
                   MPI_BYTE,
                   MBC.masterID,
                   MBC.comm,
                 &(MBC.req));
        MPI_Wait(&(MBC.req),MPI_STATUS_IGNORE);
