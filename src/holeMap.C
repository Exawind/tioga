...
void tioga::getAdaptiveHoleMap()
{
    ...
    { // using encapsulation for clean up
        ...
        for (mbi = 0; mbi < nblocks; mbi++) {
            ...
            // communicator)
            MPI_Allreduce(
                mb->bboxLocalAHM, bboxGlobal, 3, MPI_DOUBLE, MPI_MIN,
                mb->blockcomm);
            MPI_Allreduce(
                &(mb->bboxLocalAHM[3]), &(bboxGlobal[3]), 3, MPI_DOUBLE,
                MPI_MAX, mb->blockcomm);
            ...
            if (AHMOLocal.existWall != 0) {
                for (j = 0; j < 3; j++) {
                    ...
                    ds[j] =
                        fabs(AHMOLocal.extents_hi[j] - AHMOLocal.extents_lo[j]);
                }
            ...
            while (adaptMap != 0) {
                ...
                // count number of octants to refine, set refined field before
                // call octant_children
                ...
                // fill in children neighbors AFTER ALL new level octants are
                // formed
                nrefine = 0;
                for (i = 0; i < lvl->elem_count; i++) {
                    if (refineFlag[i] != 0) {
                        ...
                        octant_children_neighbors(
                            &lvl->octants[i], &new_lvl->octants[newidx + 0],
                            &new_lvl->octants[newidx + 1],
                            &new_lvl->octants[newidx + 2],
                            &new_lvl->octants[newidx + 3],
                            &new_lvl->octants[newidx + 4],
                ...
                MPI_Allreduce(
                    MPI_IN_PLACE, existWall.data(), nchildren, MPI_UINT8_T,
                    MPI_MAX, mb->blockcomm);

                // check outer boundaries: check only octants with wall tagged
                // previously
                mb->markBoundaryMapSurface(
                    OUTERNODETYPE, AHMOLocal.extents_lo, AHMOLocal.extents_hi,
                    &AHMOLocal.levels[level_id], existWall.data(),
                    existOuter.data());

                // inform all mesh-block processes with this body tag of the
                // octants flags (note the communicator)
                MPI_Allreduce(
                    MPI_IN_PLACE, existOuter.data(), nchildren, MPI_UINT8_T,
                    MPI_MAX, mb->blockcomm);
                ...
                if (AHMOLocal.nlevel >= OCTANT_MAXLEVEL && adaptMap == 1) {
                    printf(
                        "[tioga] ERROR Constructing the Adaptive Hole Map! Max "
                        "level reached!\n");
                    exit(0);
                }
            }
            // adaption complete: all ranks of this mesh-block are informed of
            // global octree
        }
                ...
                if (MBC.comm != MPI_COMM_NULL) {
                    if (MBC.id == MBC.masterID) {
                        if (AHMOLocal.existWall != 0) {
                            ...
                            memcpy(
                                meta.extents_lo, AHMOLocal.extents_lo,
                                3 * sizeof(double));
                            memcpy(
                                meta.extents_hi, AHMOLocal.extents_hi,
                                3 * sizeof(double));

                            // 2. loop each level in adaptive hole map and
                            // assemble octants
                            uint64_t elem_count = 0;
                            for (level_id = 0; level_id < meta.nlevel;
                                 level_id++) {
                                level_octant_t* lvl =
                                    &AHMOLocal.levels[level_id];
                                level_octant_coordinate_t* elvl =
                                    &AHMC.levels[level_id];
                    ...
                    // 2. communicate minimal adaptive hole map to composite
                    // ranks a. inform of meta data for map: packed into
                    // contiguous buffer
                    MPI_Bcast(
                        &meta, sizeof(ahm_meta_minimal_t), MPI_BYTE,
                        MBC.masterID, MBC.comm);

                    for (level_id = 0; level_id < meta.nlevel; level_id++) {
                        level_octant_coordinate_t* level =
                            &AHMC.levels[level_id];
                        level->level_id = level_id;

                        // communicate number of leaf octants on level
                        MPI_Bcast(
                            &(level->elem_count), 1, MPI_UINT32_T, MBC.masterID,
                            MBC.comm);
                    ...
                    // e. loop each level: check composite map wall boundaries
                    // for all mesh blocks
                    for (level_id = 0; level_id < meta.nlevel; level_id++) {
                        level_octant_coordinate_t* level =
                            &AHMC.levels[level_id];
                        ...
                        // check composite map wall boundaries for all mesh
                        // blocks
                        for (mbi = 0; mbi < nblocks; mbi++) {
                            auto& mb = mblocks[mbi];
        ...
        for (mbi = 0; mbi < nblocks; mbi++) {
            ...
            if (AHMLocal.existWall != 0U) {
                ...
                meta.nlevel = AHMOLocal.nlevel;
                memcpy(
                    meta.extents_lo, AHMOLocal.extents_lo, 3 * sizeof(double));
                memcpy(
                    meta.extents_hi, AHMOLocal.extents_hi, 3 * sizeof(double));
    ...
    for (int i = 0; i < maxtag; i++) {
        ...
        if (AHME.existWall != 0U) {
            ...
            if (MBC.comm != MPI_COMM_NULL) {
                ...
                MPI_Bcast(
                    &AHME.meta, sizeof(ahm_meta_t), MPI_BYTE, MBC.masterID,
                    MBC.comm);
                ...
                for (level_id = 0; level_id < meta.nlevel; level_id++) {
                    ...
                    // communicate number of leaf octants on level
                    MPI_Bcast(
                        &(elvl->elem_count), 1, MPI_UINT32_T, MBC.masterID,
                        MBC.comm);
...
void tioga::outputHoleMap()
{
    ...
    for (i = 0; i < nmesh; i++) {
        if (holeMap[i].existWall != 0) {
            ...
            nnodes = (holeMap[i].nx[0] + 1) * (holeMap[i].nx[1] + 1) *
                     (holeMap[i].nx[2] + 1);
            ncells =
                (holeMap[i].nx[0]) * (holeMap[i].nx[1]) * (holeMap[i].nx[2]);
            fprintf(
...
void writePointsVolume(
    FILE* fp, int level, int id, double* x, int npts1d, int type)
{
    int i, j, k;
    int pt;
    ...
    /* write zone header */
    fprintf(
        fp, "ZONE T=\"Volume_L%d_Q%d\", I=%d, J=%d, K=%d, F=POINT\n", level, id,
        npts1d, npts1d, npts1d);
    ...
    FILE* fp, int level, int id, double* x, int npts1d, int type)
{
    ...
    /* write zone header */
    fprintf(
        fp, "ZONE T=\"Volume_L%d_Q%d\",N=%d, E=%d ET=BRICK, F=FEBLOCK\n", level,
        id, 8, 1);
    ...
    if (myid == 0) {
        for (m = 0; m < nmesh; m++) {
            if (adaptiveHoleMap[m].existWall != 0U) {
                ...
                for (level = (meta.nlevel > 1) ? 1 : 0; level < meta.nlevel;
                     level++) {
                    ...
                    for (e = 0; e < L.elem_count; e++) {
                        ...
                        x[0] = meta.extents_lo[0] +
                               ds[0] * INT2DBL * L.octants[e].x;
                        y[0] = meta.extents_lo[1] +
                               ds[1] * INT2DBL * L.octants[e].y;
                        z[0] = meta.extents_lo[2] +
                               ds[2] * INT2DBL * L.octants[e].z;
                        x[1] = x[0] + dx[0];
                        y[1] = y[0] + dx[1];
                        z[1] = z[0] + dx[2];
                        ...
                        writePointsVolume(
                            file, level, e, pt_coords, 2,
                            L.octants[e].filltype);
                    }
                }
                fflush(file);
                fclose(file);
