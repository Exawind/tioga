    ...
    const double* ref_ratio, std::vector<double>& ref_coord)
{
    ...
    ref_coord[0] = (ref_ratio[0] - 0.5 >= 0)
                       ? ((ref_ratio[0] - 0.5) / p * 2 - 1)
                       : ((ref_ratio[0] + 0.5 * (2 * p - 1)) / p * 2 - 1);
    ref_coord[1] = (ref_ratio[1] - 0.5 >= 0)
                       ? ((ref_ratio[1] - 0.5) / p * 2 - 1)
                       : ((ref_ratio[1] + 0.5 * (2 * p - 1)) / p * 2 - 1);
    ref_coord[2] = (ref_ratio[2] - 0.5 >= 0)
                       ? ((ref_ratio[2] - 0.5) / p * 2 - 1)
    ...
    for (int k = 0; k < basis; k++) {
        for (int j = 0; j < basis; j++) {
            for (int i = 0; i < basis; i++) {
                ijk_stencil[ind++] =
                    std::max(-nf, std::min(start_x + i * 1, nX + nf - 1));
                ijk_stencil[ind++] =
                    std::max(-nf, std::min(start_y + j * 1, nY + nf - 1));
                ijk_stencil[ind++] =
                    std::max(-nf, std::min(start_z + k * 1, nZ + nf - 1));
            }
        }
    }
}
    ...
    bool isNodal)
{
    ...
    // left->right is along x-axis. back->front is along y-axis. bottom->top is
    // along z-axis
