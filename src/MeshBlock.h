...
private:
  ...
  std::vector<int> wbcfacenode; /** < [4*nwbcface] wall face boundary nodes (-1
                                   node: triangle face) */
  std::vector<int> obcfacenode; /** < [4*nobcface] outer face boundary nodes (-1
                                   node: triangle face) */
  ...
  int ipoint;
  int *picked; /** < flag specifying if a node has been selected for high-order
                  interpolation */
...
public:
  ...
  void setData(int btag, int nnodesi, double *xyzi, int *ibli, int nwbci,
               int nobci, int *wbcnodei, int *obcnodei, int ntypesi, int *nvi,
               int *nci, int **vconni, uint64_t *cell_gid = NULL,
  ...
  void getInterpolatedSolution(int *nints, int *nreals, int **intData,
                               double **realData, double *q, int nvar,
                               int interptype);

  void getInterpolatedSolutionAMR(int *nints, int *nreals, int **intData,
                                  double **realData, double *q, int *);
  ...
  void markBoundaryAdaptiveMap(char nodetype2tag, double extents_lo[3],
                               double extents_hi[3], level_octant_t *level,
                               uint8_t *taggedList, uint8_t *tagList);

  void markBoundaryAdaptiveMapSurfaceIntersect(
      char nodetype2tag, double extents_lo[3], double extents_hi[3],
      level_octant_t *level, uint8_t *taggedList, uint8_t *tagList);

  void markBoundaryAdaptiveMapSurfaceIntersect(
      char nodetype2tag, double extents_lo[3], double extents_hi[3],
      uint8_t level_id, uint32_t noctants, octant_coordinates_t *octants,
      uint8_t *taggedList, uint8_t *tagList);

  void markBoundaryMapSurface(char nodetype2tag, double extents_lo[3],
                              double extents_hi[3], level_octant_t *level,
                              uint8_t *taggedList, uint8_t *tagList);
  ...
  void insertAndSort(int pointid, int senderid, int meshtag, int remoteid,
  ...
  void fillReceptorDataPtr(CartGrid *cg, int cell_count, int c, int j, int k,
                           int l, int *pmap, double vol, double *xtm,
                           bool isNodal, INTEGERLIST2 *&dataPtr);
