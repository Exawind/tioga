...
void tioga::exchangeSearchData(int at_points)
{
    ...
    // Loop through recv packets and estimate search array sizes in MeshBlock
    // data structures
    ...
    //
    std::vector<int> icOffset(
        nblocks, 0); // Index of isearch arrays where next fill happens
    std::vector<int> dcOffset(
        nblocks, 0); // Index of xsearch arrays where next fill happens
    std::vector<int> rcOffset(
        nblocks, 0); // Index of res_search arrays where next fill happens
    std::vector<int> igOffset(
        nblocks, 0); // Index of gid_search where next fill happens
    for (int k = 0; k < nrecv; k++) {
        int l = 0;
        int m = 0;
...
#ifdef TIOGA_HAS_NODEGID
                std::memcpy(
                    &(mb->gid_search[goff]), &(rcvPack[k].intData[m]),
                    sizeof(uint64_t));
                m += 2;
                goff++;
#endif
