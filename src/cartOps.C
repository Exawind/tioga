...
void MeshBlock::getUnresolvedMandatoryReceptors(void)
{
    ...
    if (ihigh) {
        ...
        for (i = 0; i < nreceptorCellsCart; i++) {
            get_receptor_nodes(
                &(ctag_cart[i]), &(pointsPerCell[i]), &(rxyzCart[m]));
            m += (3 * pointsPerCell[i]);
        }
        TIOGA_FREE(iflag);
    } else {
...
void MeshBlock::findInterpListCart(void)
{
    ...
    for (irecord = 0; irecord < nsearch; irecord++)
        if (donorId[irecord] != -1) {
            ...
            //
            interpListCart[interpCount].inode =
                (int*)malloc(sizeof(int) * nvert);
            interpListCart[interpCount].weights =
                (double*)malloc(sizeof(double) * nvert);
            for (m = 0; m < nvert; m++) {
...
void MeshBlock::getInterpolatedSolutionAMR(
    int* nints,
    int* nreals,
    int** intData,
    double** realData,
