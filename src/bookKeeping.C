        ...
        for (i = 0; i < nwbc; i++) {
            if (iblank[wbcnode[i] - BASE] == HOLE) {
                printf(
                    ...
                    "----------\n");
                printf(
                    "Alarm from process %d : wall node is being tagged as a "
                    "hole %d %p\n",
    ...
    *nrecords = 0;
    ...
    for (i = 0; i < nnodes; i++) {
        ...
        if (iblank[i] < 0) {
            ...
            (*receptorResolution)[k++] =
                (resolutionScale > 1.0) ? -nodeRes[i] : nodeRes[i];
...
void MeshBlock::set_ninterp(int ninterp_input) { ninterp = ninterp_input; }
...
void MeshBlock::resetCoincident()
{
    ...
    for (i = 0; i < nsearch; i++) {
        ...
        if (iptr > -1) {
            ireset[xtag[i]] =
                std::min(ireset[xtag[i]], interpList[iptr].cancel);
        }
    }
    for (i = 0; i < nsearch; i++) {
        iptr = interp2donor[i];
        ...
        for (n = 0; n < ntypes; n++) {
            ...
            for (i = 0; i < nc[n]; i++) {
                ...
                if (ncount > 0 && ncount < nvert) {
                    for (m = 0; m < nvert; m++) {
                        if (ibltmp[inode[m]] == 0 && iblank[inode[m]] < 0) {
